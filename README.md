...
lets us use prop drilling
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
 reducers where the change of state happens while actions only deliver what the reducer has, store holds the reducer so that you can access the state with your components.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
it is middleware that helps us with asychronous function calling
4. What is your favorite state management system you've learned and this sprint? Please explain why! context api becuase it is so much easier than reduxt
